...
export const createBlog = (newBlog) => {
  return async (dispatch) => {
    try {
      ...
      dispatch(
        setNoti(
          `${newBlog.title} by ${newBlog.author} was added`,
          "success",
          5,
        ),
      );
      ...
      dispatch(
        setNoti(`Add blog unsuccessful: ${handleError(error)}`, "error", 5),
      );
    }
  };
};
...
export const addComment = (id, comment) => {
  return async (dispatch) => {
    try {
      ...
      dispatch(
        setNoti(`Unable to upload comment: ${handleError(error)}`, "error", 5),
      );
    }
  };
};
